...
void Openfile(){
    ...
    int flen=ftell(fp);
    const char* a = (char *)malloc((sizeof(char)) * flen);
...
//	gtk_text_buffer_insert(GTK_TEXT_BUFFER(buffer),&start,"a",1);
//	gtk_text_buffer_insert(GTK_TEXT_BUFFER(buffer),&start,filename,strlen(filename));
	free(a);
	return;
}
